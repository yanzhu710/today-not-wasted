...
export async function renderMine(view, ctx) {
  ...
  const setCard = h('div', { class: 'card' },
    ...
    setRow('主题', h('div', { class: 'seg', style: 'width:190px' },
      h('button', { class: settings.theme !== 'dark' ? 'on' : '', onclick: async (e) => { control.parentElement.querySelectorAll('button').forEach((b, i) => b.classList.toggle('on', i === 0)); await patchKV('settings', { theme: 'warm' }); await refreshSettings(); } }, '温暖手账'),
      h('button', { class: settings.theme === 'dark' ? 'on' : '', onclick: async (e) => { control.parentElement.querySelectorAll('button').forEach((b, i) => b.classList.toggle('on', i === 1)); await patchKV('settings', { theme: 'dark' }); await refreshSettings(); } }, icon('moon'), '夜读'))),
    setRow('动效', h('div', { class: 'seg', style: 'width:230px' },
      ['rich', 'light', 'off'].map((m, i) => h('button', { class: settings.motion === m ? 'on' : '', onclick: async (e) => { control.parentElement.querySelectorAll('button').forEach((b, j) => b.classList.toggle('on', j === i)); await patchKV('settings', { motion: m }); await refreshSettings(); } }, ['丰富', '轻', '关'][i])))),
